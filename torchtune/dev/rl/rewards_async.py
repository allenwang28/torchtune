...
async def parse_math(
    ...
    try:
        ...
        result = await asyncio.to_thread(
            extract_target_from_pred, pred, target_res, fallback_mode, extraction_mode
        )
        return result
...
async def verify_math(
    ...
    def compare_single_extraction(
        ...
        # We don't support str / sympy.Expr comparison
    ...
    async def compare_single_extraction_wrapper(g, t):
        try:
            # Run the comparison in a separate thread to avoid blocking the event loop
            return await asyncio.to_thread(compare_single_extraction, g, t)
        except Exception:
...
async def at_least_one_space_between_think_tags(
...
async def math_response_correct(
    cot: str, answer: str, potential_answer: str
) -> Tuple[float, float]:
    """Did it get the right answer?"""
    if potential_answer is None:
        return 0.0, 0.0  # (reward, success)
    gold = await parse_math(answer)
    attempt = await parse_math(potential_answer)

    if await verify_math(gold, attempt):
        return 100.0, 1.0
    ...
    for b in range(batch_size):
        for g in range(grpo_size):
            ...
            for rw_idx, reward_func in enumerate(reward_funcs):
                reward, success = await reward_func(cot, answer, potential_answer)
